...
const ν  = 1.109e-5
const U₀ = 0.15
const δ  = sqrt(2ν / ω)        # Stokes thickness
const a  = U₀ / ω              # tidal excursion scale, sets the buoyancy scale
const T_tide = 2π / ω

const zmax_δ = 50.0            # the paper's z/δ_s axis range
const z_sponge_δ = 45.0        # above this the sponge mask exceeds ~5%: shaded

cases = [("Ri0", 0.0), ("Ri500", 500.0 * ω^2), ("Ri2500", 2500.0 * ω^2)]
...
# ωt = 50.3, the paper's late time.
n_periods_plot = [1, 2, 4, 8, 15]
...
for (case, N²) in cases
    ...
    unstratified = N² == 0
    bscale = unstratified ? 1.0 : a * N²
    gscale = unstratified ? 1.0 : N²

    pa = plot(; ylabel = "z / δ_s", xlabel = unstratified ?
                  "⟨b⟩ₓᵧ  (m s⁻²)" : "⟨b⟩ₓᵧ / (a N²)",
              title = "(a) buoyancy", ylims = (0, zmax_δ), yticks = 0:5:zmax_δ,
              legend = :bottomright,
              foreground_color_legend = nothing, background_color_legend = nothing)
    pb = plot(; ylabel = "z / δ_s", xlabel = unstratified ?
                  "∂⟨b⟩ₓᵧ/∂z  (s⁻²)" : "∂⟨b⟩ₓᵧ/∂z / N²",
    ...
    for p in (pa, pb)
        # The sponge is already ~5% active here; profiles above are relaxed
        # toward the background, not freely evolving.
    ...
    unstratified || xlims!(pb, 0, 1.5)

    if unstratified
        annotate!(pa, [(0.5, 0.5 * zmax_δ,
            text("N² = 0 — no buoyancy field\nin this run: ⟨b⟩ ≡ 0",
                 9, :center, RGB(0.32, 0.32, 0.32)))])
        xlims!(pa, -1, 1); xlims!(pb, -1, 1)
    end

    Ri_label = case == "Ri0" ? "0" : case[3:end]
