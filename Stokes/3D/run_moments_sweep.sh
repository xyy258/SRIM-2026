...
# and the K_T(TKE) exponent. Follows the pattern of the earlier run_sweep.sh.
#
...
#   ./run_moments_sweep.sh stage2    geometry: N/ω = 2 across a T list (opt.)
#   ./run_moments_sweep.sh stage3    fill: every (T, N/ω) not yet done (opt.)
#   ./run_moments_sweep.sh post      re-run MixedLayerDiffusivity.jl only
#   ./run_moments_sweep.sh h0        re-print h0 from the stage-0 run
#
# Each stage is separately invokable and each is resumable: a case that finished
# leaves outputs/<tag>/.done_moments and is skipped on the next invocation.
#
# ---------------------------------------------------------------------------
# READ THIS BEFORE RUNNING STAGES 1-3
# ---------------------------------------------------------------------------
# THE DEFAULT IS ONE COLUMN: T = 10 m, N/ω ∈ {0,1,2,10}, 8 periods, which is
# 11.1 h of GPU including the spin-up.
#
# T = 10 m comes from the previous sweep's own figure 5 panels (archived under
...
# no-slip bottom; drag raises u* and deepens the layer, so stage 0 re-measures.
#
# STAGE 0 IS THE COLUMN'S OWN N/ω = 0 CASE, not a throwaway probe. It measures h0
# — the height at which TKE falls to ~1 % of its near-wall peak at peak phase —
# and is then kept and marked, so the column skips it. A pycnocline above the
# turbulent layer is never reached, and since the softplus background is
# unstratified below z = T, such a case measures nothing at all; measure_h0.jl
# prints the T to use instead. The usable window is roughly 0.3 h0 to 2 h0.
#
# DO NOT LAUNCH STAGE 3 UNTIL PANEL (c) OF STAGE 1 SHOWS THE TWO K_T ESTIMATES
# AGREEING. K_T_bulk (from wb and F_sgs) and K_T_pe (from B alone) share no code
# path; if they disagree the subgrid flux is wrong and all 22 h of stage 3 would
# measure the closure rather than the flow. This script enforces the gate — see
# `check_gate` below — and STAGE3_FORCE=1 overrides it if you have decided the
# disagreement is understood.
#
# THE BOTTOM BOUNDARY IS NOW QUADRATIC DRAG, not the no-slip every run currently
# in outputs/ used, so those runs are not comparable with these and their spin-up
# is not the right restart. `spinup` builds a fresh one under its own tag; the
# no-slip snapshots are left untouched.
#
# SHARP = 2 for every case (the new default in case_params.jl). Do NOT vary it
# per case to equalise the pycnocline cell count: that would make the initial
# pycnocline width T-dependent and confound it with T itself.
#
# PERIODS AND THE CASE LIST ARE CUT FROM THE BRIEF'S to fit swirles.sh's 12 h
# allocation: 8 periods rather than 16, and N/ω ∈ {0,1,2,10} rather than
# {0,1,2,5,10}. Measured throughput on this grid is 0.26 h/period plus ~15 % for
# the moments writer, so 36 case-periods is one submission. The full brief runs
# with PERIODS=16 SQRT_RI="0 1 2 5 10" at ~5× the wall clock. See the header of
# swirlesrun.jl, which carries the same defaults and the per-submission table.
#
# Knobs: PERIODS, SPIN_PERIODS, T_VALUES, SQRT_RI, N_PERIODS_STAGE0, CD,
...
PERIODS="${PERIODS:-8}"                    # production; 7 survive SKIP_PERIODS=1,
                                           # ~0.85 decades in t. 16 is the brief's
                                           # number and costs twice the wall clock.
SPIN_PERIODS="${SPIN_PERIODS:-5}"
STAGE0_PERIODS="${N_PERIODS_STAGE0:-$PERIODS}"   # the probe IS a production case
T_VALUES="${T_VALUES:-10}"                 # one column; see the T = 10 note above
SQRT_RI="${SQRT_RI:-0 1 2 10}"             # 0.5 dropped: Δb is tiny there and
                                           # K_T = −∫F/Δb is ill-conditioned.
                                           # 5 dropped for the budget: it sits
                                           # between 2 and 10 on a log axis.
                                           # 0 kept: passive scalar, the
                                           # unstratified-limit control.
STAGE1_T=3
STAGE2_S=2
STAGE0_T=5
STAGE0_S=0

# The grid is unchanged from the earlier sweeps, but the BOTTOM BC is not, so the
# tag carries "drag" and cannot be confused with a no-slip snapshot.
GRID_TAG="${GRID_TAG:-100x100x300_drag}"
SPIN_TAG="spinup_${GRID_TAG}"
SPIN_FIELDS="outputs/${SPIN_TAG}/TidalBL3D_${SPIN_TAG}_fields.jld2"

# ONE case keeps its 3D snapshots, so there is a route to re-analysis this sweep
# did not anticipate; every other case runs FIELDS3D=0 LIGHT_OUTPUT=1 (~1.5 GB
# each saved). The anchor is the case both stage 1 and stage 2 pass through.
...
run_case() {
    ...
    # FIELDS3D=0 LIGHT_OUTPUT=1 everywhere except the one anchor case.
    local fields3d=0
    [ "$tag" = "$FIELDS3D_CASE" ] && fields3d=1

    log "  run $tag: T = $T m, N/ω = $S, $NP periods, FIELDS3D=$fields3d"
    # DRY_RUN reports the plan, so it is checked BEFORE the spin-up prerequisite —
    # otherwise the plan cannot be inspected until the spin-up has been built.
    ...
    if [ -f "outputs/${tag}/TidalBL3D_${tag}_moments.jld2" ]; then
        # The marker, not the file, is the completion test: an interrupted run
        # still leaves a valid but truncated moments file behind.
        date > "$mark"
...
post_case() {
    ...
    # The VERIFICATION block is the deliverable, so echo it rather than burying it.
    sed -n '/^VERIFICATION/,/^  → /p' "logs/post_${tag}.log" | tail -20
}

# ---------------------------------------------------------------------------
# The stage-3 gate: do the two K_T routes agree on stage 1?
# ---------------------------------------------------------------------------
check_gate() {
    [ "${STAGE3_FORCE:-0}" = "1" ] && { log "STAGE3_FORCE=1 — gate bypassed"; return 0; }
    "$JULIA" --project=. -e '
        using JLD2, Printf
        # Wrapped in a function: at top level a `for` body is soft scope, and
        # assigning to worst/nseen there warns and silently creates locals.
        function gate(tags)
            worst, nseen = 0.0, 0
            for tag in tags
...
spinup)
    ...
    # FIELDS3D=1 is mandatory — the 3D snapshot IS the product. MOMENTS=0: the
    # spin-up is unstratified and its second moments are of no interest.
