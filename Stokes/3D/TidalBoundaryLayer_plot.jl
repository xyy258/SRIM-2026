...
# If you want an x-y slice, you can get it this way:
    ...
    #b_bottom[:,i] = b_xz[:, 1, 1];
    b_bottom[:, i] = b_xz[:, 1]; # This is the buouyancy along the bottom wall
...
# Combine the sub-plots into a single figure
    plot(b_xz_plot, c_xz_plot, layout = (2, 1), size = (1200, 400), title = [b_title c_title])

    iter == iterations[end] && close(file_xz)
end

# Save the animation to a file
mp4(anim, "Tidal Boundary Layer Plot.mp4", fps = 8) # hide
