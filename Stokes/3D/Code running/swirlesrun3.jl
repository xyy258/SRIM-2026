#!/usr/bin/env julia
# The Γ collapse test, for the strongly stratified cases.
#
# ---------------- The hypothesis ----------------
# At large N the mixing length is limited by buoyancy, l_eff = c·√TKE/N, so
#
#     K_T = √TKE · l_eff = Γ · TKE / N        with Γ a pure number.
#
# Rearranged, that is a testable statement about a single quantity:
#
...
# This is a stronger test than fitting one exponent per case, since a handful of
# exponents near 1 can each be explained away but a set of cases landing on one
# value of Γ cannot.
#
# The figure shows the distribution of Γ for each case, and the table adds a
# single number. Since log Γ = log K_T + log N − log TKE,
#
#     d(log Γ)/d(log TKE)  =  (panel-d slope) − 1
#
# exactly, so the dlnΓ/dlnE column is the deviation of the exponent from 1, read
# as a residual rather than off a fitted line.
#
# ---------------- Two data formats ----------------
#   new     outputs/*/mixing_*.jld2 from MixedLayerDiffusivity.jl, carrying the
#           full flux F_b = ⟨w′b′⟩ + F_sgs.
#   legacy  the archived */output_*/*_profiles.jld2 sweeps, for which F_sgs was
#           never written. K_T there is the resolved flux only and so is biased
#           low, and Γ with it. An offset between the two groups is therefore an
#           artefact rather than physics. Legacy cases are drawn as open markers
#           and flagged in the table.
#
# USAGE   GKSwstype=100 julia --project=. swirlesrun3.jl
# ENV
#   SQRT_RI_MIN   keep cases with N/ω at or above this  (default 5)
...
# --- legacy format ----------------------------------------------------------
# Only Ri is needed from the folder name: Γ uses N = √Ri·ω, and h and the mask
# use N²_ref = Ri·ω². The domain height never enters, so one loader serves every
# archived sweep whatever its Lz or background shape.
function load_legacy(root)
    ...
    for d in sort(readdir(root; join = true))
        ...
        wb         = grab("wb")
        # Average(w^2) with w on Faces lands on Faces, so interpolate onto the
        # Centers rather than truncating.
        ww_c = [0.5 * (ww[k, n] + ww[min(k + 1, size(ww, 1)), n])
                for k in 1:length(zc), n in 1:nt]
        TKE = 0.5 .* ((uu .- U .^ 2) .+ (vv .- V .^ 2) .+ ww_c)
        N²_ref = Ri * ω^2
        G = zeros(length(zf), nt)                    # ∂B/∂z, computed onto Faces
        for n in 1:nt, k in 2:length(zc)
            G[k, n] = (B[k, n] - B[k-1, n]) / (zc[k] - zc[k-1])
        end
        G[1, :] .= G[2, :]
        # Legacy runs have no F_sgs, so under H_DEF=flux only the resolved part
        # is used — the same caveat their K_T already carries.
...
# Γ = K_T·N/TKE, one value per sample kept.
function gamma_of(c)
...
# The collapse in one figure: a median dot, an interquartile bar and a 10–90
# whisker per case, ordered by N/ω. If K_T = Γ·TKE/N with Γ a pure number, every
# column sits at the same height whatever N. A column away from the dashed
# pooled median is a case the closure does not cover.
#
# The whisker within a case is turbulent intermittency, not uncertainty in Γ:
# the median of several hundred samples is much better determined than the
# spread suggests. What matters here is the alignment of the dots.
...
# Labelled in the legend rather than on the line itself, where the text would
# land on top of the first case's marker.
