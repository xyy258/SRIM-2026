#!/usr/bin/env julia
# =============================================================================
# swirlesrun6.jl — the log-TKE profile is a straight line. How straight, and
#                  what does the tide do to it?
#
# On the log axis of swirlesrun5.jl's movies the profiles stay very nearly
# straight while they rise and fall. A straight line on log-x / linear-z is
#
#     TKE(z) = TKE₀ exp(−z / Lₑ)                                            (1)
#
# so the eye is reading an exponential decay with ONE length per case. This
...
#                                       composited onto one tidal cycle
#
...
# oscillates in log space: the line the eye interpolates through a wobbling
# curve is the mean of the logarithm. (The arithmetic-mean fit is reported in
# the log as a check — it leans on the flood/ebb peaks and sits slightly
# steeper.)
#
# THE FIT WINDOW is z ∈ [ZFIT_MIN, T], below the pycnocline. ZFIT_MIN = 0.2 m
# ≈ 1.4 δ_s excludes the near-wall peak at z ≈ 0.03 m, which is not part of the
# decay (1): TKE turns over there because it must vanish on the no-slip bed.
# The log also fits the whole plotted column and a two-segment line, which
# locates the knee that a single line is being punished for.
#
# ERRORS ARE QUOTED IN DECADES, not as R². A residual of 0.01 dec means the
# line is within 2.3% of the mean profile at that height, and that statement
# survives the change of variable, which "R² = 0.999" does not.
#
# THE ANIMATION shows log₁₀TKE(z,t) − ⟨log₁₀TKE⟩_t, i.e. the perturbation about
# the time average, at every height — not about the fitted line, so that what
# moves is separated from what the fit already gets wrong (the latter is the
# error figure, and the two add). The cycles are FOLDED onto one: the response
# is phase-locked at 2ω, so a composite is a fair summary and the ±1σ ribbon
# across cycles says how fair.
#
# INPUT is *_moments.jld2; the TKE definition, boxcar and analysis window are
# taken unchanged from swirlesrun5.jl, so these numbers describe the movies and
# not a re-derived quantity that happens to share a name.
#
# USAGE
...
function load_tke(T, s)
    ...
    TKE = boxcar(TKE_raw, nh)                                    # smooth second
...
function fitline(z, y)
    ...
    r = yy .- (a .+ b .* zz)
    rms = sqrt(mean(r .^ 2))
    # Standard slope error. Residuals are correlated in z — neighbouring cells
    # are not independent samples — so this UNDERSTATES the uncertainty. Quoted
    # to show the slope is not ambiguous, not as a confidence interval.
    se_b = rms * sqrt(length(k) / (length(k) - 2)) / sqrt(Szz)
    return (; a, b, rms, se_b, n = length(k),
              R2 = 1 - sum(r .^ 2) / sum((yy .- ȳ) .^ 2), maxres = maximum(abs.(r)))
end

Le_of(b) = -1 / (b * log(10))                    # decades/m → e-folding metres

# Where does the single line fail? Continuous two-segment fit, breakpoint by
# exhaustive search. Not a model of anything — a measurement of the knee.
...
# Mean + 1st + 2nd tidal harmonic. Phase quoted as the fraction of a period by
# which the harmonic's maximum lags t = 0, which is slack water with U∞ rising.
...
function analyse(c)
    ...
    # With ζ = z − z̄ the residual about the time-mean line splits orthogonally,
    #     r(z,t) = A(t) + B(t)ζ + s(z,t),   ⟨r²⟩_z = A² + B²⟨ζ²⟩ + ⟨s²⟩,
    # into a rigid slide of the whole profile (A), a pivot about mid-window (B)
    # and genuine loss of straightness (s). The three are a closed budget.
    ...
    # The tilt is NOT reportable as Lₑ(t): B is comparable to b itself, so the
    # instantaneous slope crosses zero near slack — when the profile over the
    # window is momentarily flat — and −1/(b ln10) has a pole there. The drop
    # across the window stays finite and is what the movie shows.
    D_t = .-(fit.b .+ B) .* Δz
    D̄   = -fit.b * Δz

    hA = harmonic_fit(c.times, A)

    # Why the line pivots: the 2ω response at each height. If the top of the
    # window hears the tide later than the bed, the profile cannot slide
    # rigidly — it must tilt.
    lagz = fill(NaN, length(c.z))
    for k in kplot
        h = harmonic_fit(c.times, view(L, k, :))
        h === nothing || (lagz[k] = h.lag2)
    end
    for k in kplot[2:end]         # unwrap: lag2 lives in [0, ½) and wraps
    ...
    # --- composite: the perturbation about the time mean, folded onto one cycle
...
function fig_mean(analysed)
    ...
    for T in T_values
        ...
        # Snapped to whole decades with one labelled tick each: GR left to itself
        # labels every second decade, and the reader is then counting gridlines.
...
function fig_error(analysed)
    ...
    for T in T_values
        R = get(analysed, T, Any[]); isempty(R) && continue
        # The axis is set by the residual INSIDE the window — that is what this
        # figure is for. Outside it the line is an extrapolation and misses by
        # whole decades, so those curves leave the frame; that they leave it at
        # all is the message, and the mean figure shows where.
...
# ---------------------------------------------------------------------------
# Figure 3 — the perturbation, as a movie of one composite cycle
# ---------------------------------------------------------------------------
function anim_dev(T, R)
    ...
    # must look twice as wide, which it cannot on an axis that rescales with it.
    xm = 0.0
    for r in R, i in 1:NPHASE, k in r.kplot
        isfinite(r.dev[k, i]) && (xm = max(xm, abs(r.dev[k, i])))
    end
    xm = 1.1xm       # the ±1σ ribbon may clip at the extremes; the curve never does
    ...
    function draw(i)
        ...
        # A factor axis on top: readers think in "×2", not "0.3 decades".
        for f in (0.5, 2.0)
            d = log10(f)
    ...
    if PREVIEW
        ...
        savefig(draw(cld(NPHASE, 4)), out)          # a quarter through: peak flow
