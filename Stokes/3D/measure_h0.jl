...
# h0, the height of the turbulent layer, measured from a *_moments.jld2 file.
#
# Stage 0 of the moments sweep exists to produce this number: the T values of the
# later stages are only meaningful relative to how far the turbulence reaches,
# since a pycnocline placed above the turbulent layer is never eroded and the
# case measures nothing. Once h0 is known, use T ≈ {0.4, 0.7, 1.2, 2.0} × h0.
#
# Definition: at the phase of the last full period where the near-wall TKE peaks,
# h0 is the first height above that peak at which TKE has fallen to TKE_FRAC
# (1 % by default) of it. This is not the mixed-layer height h used elsewhere:
# h0 measures where the turbulence reaches, h where the stratification has been
# sharpened. Both are reported so the two can be compared.
#
...
# Decompose each sample separately, never time-averaging first: the other order
# folds the variance of the tidal mean flow into "TKE" (see Moments.jl).
TKE = 0.5 .* ((uu .- U .^ 2) .+ (vv .- V .^ 2) .+ ww)

# The last full period only, since earlier ones still carry the restart
# transient.
t_end = times[end]
last_period = findall(n -> times[n] >= t_end - T_tide, 1:nt)
length(last_period) < 3 && (last_period = collect(1:nt))

# The peak phase is the sample in that period with the largest near-wall TKE.
# The search stops at z ≤ H_PEAK: over the whole column a spurious maximum aloft
# (decaying initial noise, internal waves, or the sponge) could win, and h0 would
# then be measured from the wrong level. The height of the peak is printed, so a
# bad H_PEAK is visible.
...
# This is a descending crossing, so first_crossing does not apply; the linear
# interpolation in z follows the same convention.
...
# A single fixed fraction is brittle: if TKE has not fallen that far by the top
# of the column the answer is NaN, which blocks the later stages entirely. So try
# the requested fraction and then progressively looser ones, reporting which one
# actually gave the answer.
...
# The mixed-layer height at the same instant, for comparison. N²_ref is taken
# from the far field rather than passed in, so this script needs no case options.
...
# Whether that peak is a single interface or the tallest of several bumps. In the
# N/ω = 0 cases this script is usually run on, the buoyancy is passive and the
# gradient above the layer is noise, so h_ml is then not a height of anything.
...
# A plain number for the sweep driver to read.
