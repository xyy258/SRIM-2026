#!/bin/bash
# =============================================================================
# run_h_definitions.sh — the whole K_T / TKE analysis, three times, once per
# definition of the mixed-layer height h, into three self-contained folders.
#
...
#
# Each folder gets figures/ and logs/ with IDENTICAL filenames, so the three can
# be compared by flipping between folders. The mixing_*.jld2 data files all share
# outputs/<tag>/ and are told apart by a suffix (_hcross, _hgrad, _hflux) and by
# the h_def field stored inside them; every downstream script reads only the
# files matching its own H_DEF, so the three passes cannot contaminate each other
...
run_one() {
    ...
    # 1. K_T and h themselves, per case. Writes mixing_<tag><MIX_SUFFIX>.jld2
    #    next to the moments, and K_T_<tag>.png into this definition's folder.
    ...
    # 2. everything that reads those files back
