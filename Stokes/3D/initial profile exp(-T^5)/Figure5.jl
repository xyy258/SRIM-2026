...
# Figure 5 for the T sweep (deviates from Gayen et al.; see case_params.jl).
# Linear background with an initial bottom mixed layer of depth T carved in.
...
# shows as a family of curves. One PNG per case: figures/Figure5_T<T>_<Ri>.png.
#
...
tint(T) = isinteger(T) ? string(Int(T)) : replace(string(T), "." => "p")  # T=4.5 → "T4p5"
...
for T in T_values, (Ri, ricase) in Ri_values
    ...
    # Initial profile at t = 0 (this case's T): a bottom mixed layer of depth T
    # carved into linear stratification, bᵢ = N∞² z [1 − exp(−0.2 (z/T)^5)] — the
    # state the time series evolve away from. Normalized like the data: buoyancy
    # by a·N∞², gradient by N∞². Drawn first so the curves sit on top.
    b_init_norm(z) = z * (1 - exp(-0.2 * (z / T)^5)) / a                       # bᵢ / (a N∞²)
    G_init_norm(z) = 1 - exp(-0.2 * (z / T)^5) * (1 - (z / T)^5)               # ∂bᵢ/∂z / N∞²
    plot!(pa, b_init_norm.(zc[kc]), zc[kc]; color = :black, linestyle = :dash,
          linewidth = 1.5, label = "initial (t = 0)")
    plot!(pb, G_init_norm.(zg[kg]), zg[kg]; color = :black, linestyle = :dash,
          linewidth = 1.5, label = "initial (t = 0)")
    ...
    fig = plot(pa, pb; layout = (1, 2), size = (950, 620), leftmargin = 5Plots.mm,
               ...
               plot_title = @sprintf("T = %.1f m, Ri = %d — thermal field (δ = u*/f = %.2f m)", T, Ri, δ),
