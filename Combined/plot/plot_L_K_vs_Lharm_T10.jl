...
#
#     1/L_harm = 1/L_N + 1/L_s          L_N = √TKE/N,  L_s = √TKE/S
#
...
# Written as 1/L_harm = 1/L_N + 1/L_s, the two terms are resistances in series and
# their shares add to one:
#
#     w_N = L_harm/L_N       w_s = L_harm/L_s       w_N + w_s = 1
#
# w_s is the fraction of 1/L_harm contributed by the shear. w_s → 1 is
...
#
# USAGE  cd Combined && GKSwstype=100 julia --project=. plot/plot_L_K_vs_Lharm_T10.jl
...
jldopen(CACHE, "r") do io
    for (nm, dst) in (("stokes", stokes), ("ekman", ekman))
        for r in io["$nm/ratios"]
            ...
            L_harm = 1 ./ (1 ./ L_N .+ 1 ./ L_s)
            push!(dst, (r = r, L_K = io["$g/L_K"], L_N = L_N, L_s = L_s, L_harm = L_harm,
                        w_N = L_harm ./ L_N, w_s = L_harm ./ L_s))
        end
    end
end
...
# geometric mean of L_K/L_harm and the rms is the spread of that ratio in log space.
function propfit(cs)
    lr = fin([log(med(c.L_K)) - log(med(c.L_harm)) for c in cs])
    a = mean(lr)
...
function powfit(cs)
    lx = [log(med(c.L_harm)) for c in cs]; ly = [log(med(c.L_K)) for c in cs]
...
function satfit(cs)
    x = [med(c.L_harm) for c in cs]; y = [med(c.L_K) for c in cs]
...
say("")
say("the shear's share of 1/L_harm, w_s = L_harm/L_s.  w_s > 1/2 means the shear scale")
say("is the smaller of the two and the mixing is shear-limited")
say("  flow    r       L_N (m)   L_s (m)   L_harm (m)   L_K (m)     w_N     w_s")
for (nm, cs) in (("Stokes", stokes), ("Ekman ", ekman)), c in cs
    say(@sprintf("  %s %-5g %9.4f %9.4f %9.4f %9.4f %7.3f %7.3f%s",
                 nm, c.r, med(c.L_N), med(c.L_s), med(c.L_harm), med(c.L_K),
...
pa = plot(xscale = :log10, yscale = :log10,
          xlabel = L"L_{\mathrm{harm}} = 1/(1/L_N + 1/L_s) \ \ (\mathrm{m})",
...
xs = [med(c.L_harm) for c in BOTH]
xx = exp.(range(log(minimum(xs) / 1.7), log(maximum(xs) * 1.7); length = 300))
plot!(pa, xx, PR.f.(xx); color = :grey55, lw = 1.4, ls = :dot,
      label = latexstring(@sprintf("L_K = %.3f\\,L_{\\mathrm{harm}} \\ \\ (\\mathrm{proportional,\\ rms}\\ %.0f\\,\\%%)", PR.A, PR.rms)))
SA.pinned || plot!(pa, xx, SA.f.(xx); color = :black, lw = 2.6,
      label = latexstring(@sprintf("L_K = %.2f(1 - e^{-L_{\\mathrm{harm}}/%.2f}), \\ \\ \\mathrm{rms}\\ %.1f\\,\\%%", SA.L, SA.x0, SA.rms)))

# Quartile bars first, without markers, so the coloured markers sit on top.
for cs in (stokes, ekman), c in cs
    x = med(c.L_harm); y = med(c.L_K)
    scatter!(pa, [x], [y]; xerror = ([x - qlo(c.L_harm)], [qhi(c.L_harm) - x]),
...
for c in BOTH
    annotate!(pa, med(c.L_harm), med(c.L_K) / 1.45,
              text(latexstring(@sprintf("%g", c.r)), 5, :grey30, :center))
end
...
hline!(pb, [0.5]; color = :black, lw = 1.2, ls = :dash,
       label = L"L_s = L_N \ \ (\mathrm{the\ two\ scales\ equal})")
...
# GR's mathtext has no \text, so the hyphens in "shear-limited" would come out
# as minus signs. The wording avoids them.
annotate!(pb, 3.0, 0.545, text(L"\mathrm{shear\ limits\ the\ mixing\ above}", 6, :grey30, :left))
annotate!(pb, 3.0, 0.455, text(L"\mathrm{stratification\ limits\ it\ below}", 6, :grey30, :left))

f = plot(pa, pb; layout = (1, 2), size = (1250, 560),
         plot_title = L"T = 10\,\mathrm{m},\ z = h:\ \ L_K\ \mathrm{against}\ L_{\mathrm{harm}},\ \mathrm{and\ where\ each\ scale\ limits\ it}",
         left_margin = 6Plots.mm, bottom_margin = 6Plots.mm, top_margin = 3Plots.mm,
         right_margin = 4Plots.mm)
o = joinpath(FIGDIR, "L_K_vs_Lharm_T10.png")
savefig(f, o)
say("")
say("wrote $o")

mkpath(joinpath(HERE, "logs"))
open(joinpath(HERE, "logs", "plot_L_K_vs_Lharm_T10.log"), "w") do io
